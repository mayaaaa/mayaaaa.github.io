...
- 4.1節: ポアソン方程式の学習
  ...
  - このような最適化を行う (ポアソン方程式を解く)ことで元の画像を再構成できる
    - 目的関数に含まれる $$\int_{}$$は画像の場合全ピクセルについての和
...
- 4.2節: 符号付き距離関数を用いた点群の表現学習
  - 符号付き距離関数 (SDF)を点群にfitさせる
    ...
    - このタスクは空間的微分 $$|\Delta_{\bf x}\Phi|$$が1という制約の下でEikonalの境界値問題を解くことに等しい
    - 目的関数は3つの積分で書き下せる
      - 全ての点集合 $$\Omega$$について$$|\Delta_{\bf x}\Phi|=1$$
        - SDFは最寄りの点への距離なのでその微分はその点の逆方向を示す
        - ある点を1空間単位ずらしたらSDFは1大きくなる->微分は1になっているはず
      - 境界面上の点の集合 $$\Omega_0$$についてSDFが0に近づくような制約+SDFの微分と境界面の微分が平行になるような制約
      - 境界面以外の点集合 ($$\Omega \\ \Omega_0$$)が0から離れるような制約 
