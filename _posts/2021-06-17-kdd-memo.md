...
- 先行研究は過去の履歴を使うもの, 未来の気象データを使うもの, 物理モデルの出力を使うものの3つ
  ...
  - 台風が継続的にトラッキングされている状況では直近の軌跡を参考にモデルをアップデートすできる手法が望ましい
...
- 目的関数を軌跡予測の誤差 $$\mathcal_{tra}$$と大きさ予測の誤差 $$\mathcal_{int}$$の和で定義
...
- 弁護士がある裁判の結果に対してプラスに働く確率 $$P(\theta)$$を $$P(\theta)=1/(1+e^{-Q\cdot (Max\Theta-Min\Theta)(\theta-\beta)})$$
  ...
  - $$\theta$$と $$\beta$$については二通りのモデル化を検討
    1. $$\theta$$は弁護士の習熟度で $$\beta$$はケースの難しさ
    2. $$\theta$$は弁護側チームの習熟度で $$\beta$$は原告側の習熟度
...
### Temporal Graph Signal Decomposition  [PDF](https://arxiv.org/abs/2106.13517)
